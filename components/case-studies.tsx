...
export function CaseStudies() {
  return (
    <section id="work" className="stacked-section py-20 sm:py-24">
      <div className="section-shell">
        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-4 text-3xl font-semibold tracking-tight text-ink sm:text-5xl">
              Practical systems for real teams.
            </h2>
          </div>
          <Link
            href="/work"
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-3">
          {caseStudies.map((study) => (
            <article
              key={study.title}
              className="group rounded-xl border border-line bg-card/95 p-5 shadow-card transition duration-300 hover:-translate-y-1 hover:border-accent/50 sm:p-6"
            >
              ...
              <p className="mt-4 text-sm leading-6 text-muted">{study.mobileSummary}</p>
              <p className="mt-5 border-t border-line pt-5 text-sm font-medium leading-6 text-ink">
                {study.result}
              </p>
            </article>
          ))}
